...
| `--type, -T` | 否 | 文章类型：`newspic`（默认）或 `news` |
| `--images, -i` | 否 | 图片路径列表（newspic 模式必填），支持 glob，最多 20 张，PNG/JPEG/JPG/GIF |
| `--theme` | 否 | 排版主题（news 模式），默认 `default` |
| `--hl-theme` | 否 | 代码高亮主题（news 模式），如 `dracula`、`atom-one-dark` |
| `--theme-file, -f` | 否 | 自定义主题 CSS 文件路径（与 `--theme` 配合使用） |
...
| 参数 | 必填 | 默认值 | 说明 |
|------|------|--------|------|
| `--md, -m` | 是 | — | Markdown 文件路径 |
| `--output, -o` | 否 | `{文件名}.html` | 输出 HTML 文件路径 |
| `--theme` | 否 | `default` | 排版主题 |
| `--hl-theme` | 否 | 默认主题 | 代码高亮主题，如 `dracula`、`atom-one-dark` |
| `--theme-file, -f` | 否 | — | 自定义主题 CSS 文件路径（需同时指定 `--theme <名称>`） |
| `--open, -O` | 否 | — | 渲染后用默认浏览器打开 |
...
| `--port, -p` | 否 | 3000 | 监听端口 |
| `--host` | 否 | 0.0.0.0 | 监听地址 |
| `--app-id` | 否 | 凭证配置 | 微信 APP_ID |
| `--app-secret` | 否 | 凭证配置 | 微信 APP_SECRET |
...
| `--hl-theme` | 否 | 默认 | 默认代码高亮主题 |

**功能特性：**

- **主题切换** — 内置 9 个 Wenyan 主题（default、orangeheart、rainbow、lapis、pie、maize、purple、phycat、nord），也支持下拉选择
- **代码高亮主题** — 内置 9 个 hljs 代码高亮主题（Atom One Dark/Light、Dracula、GitHub、GitHub Dark、Monokai、Solarized Dark/Light、Xcode），Wenyan 主题下拉旁选择
...
### 通过微信凭证直接发布（需已配置 APP_ID / APP_SECRET）
...
```bash
wx-newspic render --md article.md --theme orangeheart --hl-theme dracula --open
wx-newspic render --md article.md --theme nord --hl-theme atom-one-dark --output preview.html
```
...
| 样式定制 | `skills/style-customizer/` | 公众号文章样式定制技能，提取视觉参数 → 生成主题 → 质量门禁 → 预览 |
...
**解决：**
1. 通过 `--server` 参数指定中转服务器地址
2. 或设置环境变量 `WECHAT_SERVER_URL` / `WX_NEWSPIC_SERVER`
3. 或运行 `wx-newspic credential set --server <url>` 持久化配置
...
**解决：**
1. 检查中转服务器日志，查看微信 API 返回的详细错误
2. 确认 access_token 有效（运行 `wx-newspic credential check`）
3. 检查文章内容是否触发微信内容安全审核
4. 确认文章标题不超过 32 字、作者不超过 16 字、摘要不超过 128 字
