...
根据提取的 `inlineAccents.isDarkBackground` 选择对应 hljs 主题：

| 文章背景 | 推荐 `--hl-theme` | 备选 |
|---------|-------------------|------|
| 深色 (`isDarkBackground: true`) | `atom-one-dark` | dracula, monokai |
| 浅色 (`isDarkBackground: false`) | `atom-one-light` | github, xcode |

主题 CSS 文件注释中注明推荐的 hljs 主题：`/* hljs-theme: atom-one-dark */`。

使用发布/渲染命令时，手动指定 `--hl-theme` 参数；如果保留默认，wenyan-core 会使用内置的默认高亮主题。
